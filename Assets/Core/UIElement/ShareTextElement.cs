...
namespace huqiang.UI
{
    public class ShareTextElement:EmojiElement
    {
        ...
        public void UpdateMesh(ShareText shareText)
        {
            var child = model.child;
            if (Collector != null)
                child = Collector.child;
            for (int i = 0; i < child.Count; i++)
                UpdateMesh(child[i],shareText);
        }
        public List<UIVertex> UpdateVertex()
        {
            var vert = new List<UIVertex>();
            var child = model.child;
            if (Collector != null)
                child = Collector.child;
            for (int i = 0; i < child.Count; i++)
                UpdateVertex(child[i], vert, Vector3.zero, Quaternion.identity, Vector3.one);
            return vert;
        }
        ...
        static void UpdateMesh(ModelElement child, ShareText ori)
        {
            if (child.activeSelf)
            {
                var stc = child.GetComponent<ShareTextChildElement>();
                ...
                for (int i = 0; i < child.child.Count; i++)
                {
                    UpdateMesh(child.child[i], ori);
                }
            }
        }
        static void UpdateVertex(ModelElement child, List<UIVertex> vertices, Vector3 position, Quaternion quate, Vector3 scale)
        {
            var pos = child.data.localPosition;
            Vector3 p = quate * pos;
            Vector3 o = Vector3.zero;
            ...
            if (child.activeSelf)
            {
                ...
                for (int i = 0; i < child.child.Count; i++)
                {
                    UpdateVertex(child.child[i], vertices, o, q, s);
                }
            }
        }
    }
