...
namespace huqiang.Data
{
    ...
    public class BlockBuffer<T> : BlockBuffer, IDisposable where T : unmanaged
    {
        ...
        /// 
        /// </summary>
        /// <param name="blockInfo">BlockInfo<byte></param>
        /// <param name="len"></param>
        /// <returns></returns>
        public unsafe bool RegNew(ref BlockInfo blockInfo, int len)
        {
            byte* bp = (byte*)ptr;
            int block = len / blockSize;
            if (len % blockSize > 0)
        ...
        lable:;
            ...
            len = block * blockSize;
            blockInfo = new BlockInfo(ID, index, len, blockSize * eSize);
            usage += block;
            return true;
        }
        /// <summary>
