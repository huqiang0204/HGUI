...
namespace huqiang.Helper.HGUI
{
    ...
    public class HGUIManager
    {
        ...
        {
            GameBuffer.RecycleGameObject(game.gameObject);
        }
        public static void RecycleChild(UIElement game)
        {
            GameBuffer.RecycleChild(game.gameObject);
        }
        public static void RecycleChild(UIElement game, string[] keep)
        {
            GameBuffer.RecycleChild(game.gameObject, keep);
        }
        public static UIElement Clone(FakeStruct fake, Initializer initializer = null)
        {
            var go = GameBuffer.Clone(fake, initializer);
            if(go!=null)
            {
                var ui = go.GetComponent<UIElement>();
#if UNITY_EDITOR
                if (ui == null)
                    Debug.LogError("模型中没有包含UI元素:"+ go.name);
#endif
                if (ui == null)
                {
                    ui = go.AddComponent<UIElement>(); 
                }
                return ui;
            }
            return null;
